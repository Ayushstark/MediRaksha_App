...
async def book_slot(request: BookSlotRequest, auth=Depends(get_current_user_id)):
    ...
    try:
        slot = await slots_collection.find_one({"_id": ObjectId(request.slotId)})
        ...
        await slots_collection.update_one({"_id": ObjectId(request.slotId)}, {"$set": {"status": "available"}})
        raise HTTPException(status_code=500, detail=str(e))
